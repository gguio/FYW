...
pi0_phi: $(PYTHIA) $$@.cc
ifeq ($(ROOT_USE),true)
	$(CXX) funcs.cc $@.cc new_funcs.cc -o $@ -w $(CXX_COMMON) \
	 `$(ROOT_CONFIG) --cflags --glibs`
else
	$(error Error: $@, new_funcs.cc and funcs.cc requires ROOT)
endif

funcs: $(PYTHIA) $$@.cc
ifeq ($(ROOT_USE),true)
	$(CXX) $@.cc pi0_phi.cc new_funcs.cc -o pi0_phi -w $(CXX_COMMON) \
	 `$(ROOT_CONFIG) --cflags --glibs`
else
	$(error Error: $@, new_funcs.cc and funcs.cc requires ROOT)
endif
...
new_funcs: $(PYTHIA) $$@.cc
ifeq ($(ROOT_USE),true)
	$(CXX) $@.cc pi0_phi.cc funcs.cc -o pi0_phi -w $(CXX_COMMON) \
	 `$(ROOT_CONFIG) --cflags --glibs`
else
	$(error Error: $@, new_funcs.cc and funcs.cc requires ROOT)
endif
...
ifeq ($(FASTJET3_USE)$(ROOT_USE),truetrue)
	$(CXX) $@.cc -o $@ -w $(FASTJET3_INCLUDE) $(CXX_COMMON)\
	 $(CXX_COMMON) $(FASTJET3_LIB) `$(ROOT_CONFIG) --cflags --glibs`
else
	$(error Error: $@ requires FASTJET3 and ROOT)
endif
